...
!! This specific README file will also contain a lot of information about Java,
FRC, and computers in general. Ideally, people who have never used Java before
can understand my madness. !!

Excluding collections, every class in the core project is public and not final.
This allows people to extend and override whatever they need in a scenario that
the core has not been prepared for.
...
**collection**:
Ever used an `ArrayList`? That's a collection. It stores multiple values of the
...
**svc**:
Code for dealing with or being the supervising laptop (driver's station).

**systems**:
Subsystem interfaces and classes for a robot. A subsystem could literally be
anything that does stuff. E.g., a shooter, a funnel, the drivetrain itself.
...
1. [WPILib Javadoc](https://github.wpilib.org/allwpilib/docs/release/java/index.html)
    is Java's documentation system. It shows you every package, class, method,
    field, etc. It also has the respective documentation comments.
2. [FRC Glossary](https://docs.wpilib.org/en/stable/docs/software/frc-glossary.html)
    has definitions of weird terms like "RSL".
3. [Command-Based Programming](https://docs.wpilib.org/en/stable/docs/software/commandbased/index.html)
    is like functional programming, but more object-oriented.
...
5. [Telemetry](https://docs.wpilib.org/en/stable/docs/software/telemetry/index.html)
    is recording and sending real-time data for analysis. Analysis could be
    anything from debugging to optimizing.
...
13. [Zero-to-Robot](https://docs.wpilib.org/en/stable/docs/zero-to-robot/introduction.html)
    article from WPI. It shows you most of what you need to know to get started
    programming. Most links have been taken from this.
...
14. [Basic Programming](https://docs.wpilib.org/en/stable/docs/software/basic-programming/index.html)
    is just a bunch of random things you probably want to know.

#### Misc
15. [Driver Station](https://docs.wpilib.org/en/stable/docs/software/driverstation/index.html)
    is a piece of software made by FIRST. It handles actually running the robot.
    In 2027, FIRST will release a cross-platform driver station, which we will 
    be using.
...
18. [Networking](https://docs.wpilib.org/en/stable/docs/networking/networking-introduction/index.html)
    and communication between the driver station and robot.
...
### Floating Point
Computers hate decimals. So some people many years ago figured out a way of
expressing decimals in a fairly fast way: floating point numbers. There are
currently two ways of dealing with floating point numbers, and those are `float`
and `double`.

Floats are 32 bits. Fairly precise at small numbers and okay at large numbers.
They are also extremely fast to do math operations on.

Doubles are 64 bits. Extremely precise at large and small numbers... But they
are quite slow to do math operations on.

Most FRC code uses only doubles. Here we mix floats and doubles:
Floats are good for rotation, such as the rotational motor for a swerve. They
deal best in small numbers, so we do **everything in radians**. Except for debug.
Doubles are good for higher numbers, so we use them for speeds like mph.
...
and with small compiled bytecode. For example, instead of a function described
as `int add(int, int)`, it can be described as `add:(II)I`.
...
```java
class C implements B<C>  {   }      // error: C does not fulfill generic Self extends B<Self> & A
...
### Static Constructors
If a class constructor is what properly sets up class fields and whatnot, then
...
at compile-time. `static final String x = "something";`. This is because strings
...
However, the Java virtual machine is also quite intelligent. If you access
`Conf.config`, then the Java virtual machine can often optimize the
`getstatic org/team4153/core/config/Conf.config` instruction (not how it
appears in actual class files) into the literal address of the field.
...
function is run enough times, then the JVM will compile it to machine code.
